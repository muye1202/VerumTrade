# opentrace/graph/opentrace_graph.py
...
from opentrace.agents import *
from opentrace.default_config import DEFAULT_CONFIG
from opentrace.agents.utils.memory.memory import FinancialSituationMemory
...
from opentrace.agents.utils.agent_runtime.tool_cache import create_cache_aware_tool_node
...
class OpenTraceGraph:
    ...
    def _log_state(self, trade_date, final_state):
        ...
        # Save to file
        directory = Path(f"eval_results/{self.ticker}/OpenTraceStrategy_logs/")
        directory.mkdir(parents=True, exist_ok=True)

        with open(
            f"eval_results/{self.ticker}/OpenTraceStrategy_logs/full_states_log_{trade_date}.json",
            "w",
            encoding="utf-8",
        ) as f:
