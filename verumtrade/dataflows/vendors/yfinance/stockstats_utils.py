...
class StockstatsUtils:
    ...
    def get_stock_stats(
        ...
        if not online:
            ...
            start_dt = curr_dt - pd.DateOffset(years=15)
            ...
            configured_cache_prefix = "YFin" if vendor != "alpaca" else "Alpaca"
            data_file = os.path.join(
                config["data_cache_dir"],
                f"{symbol}-{configured_cache_prefix}-data-{start_date}-{end_date}.csv",
            )
                ...
                if vendor == "alpaca":
                    ...
                    if bars_df is None or getattr(bars_df, "empty", False):
                        data = yf.download(
                            symbol,
                            start=start_date,
                            end=(curr_dt + pd.DateOffset(days=1)).strftime("%Y-%m-%d"),
