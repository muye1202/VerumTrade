...
from verumtrade.agents.analysts.discovery_lane import merge_workbench_metrics, record_tool_call_links
...
from verumtrade.agents.utils.market_data.bundle_tools import get_catalyst_event_bundle
...
from verumtrade.schemas.catalyst_events import CatalystEventBundle, CatalystEventReport
...
- Some `macro_events` carry an `event_type` tagging a *narrative* catalyst: `policy_trial_balloon` (a floated/walked-back policy idea — windfall tax, "citizen dividend" — attacking the boom thesis), `regulatory_narrative` (export controls / antitrust / tariffs / probe), `peer_guidance` (a peer's soft/cut/not-raised guidance tone reading through to the basket), `positioning_unwind` (parabolic / crowded-trade / momentum-rotation language). These are exactly the soft, second-order, foreign-jurisdiction triggers the single-ticker event feed misses — do NOT discount them just because there is no dated, company-specific event for {ticker}. When one is present on a crowded/extended name, treat it as elevated pullback risk in `event_risk_rating` and `risk_controls`.
