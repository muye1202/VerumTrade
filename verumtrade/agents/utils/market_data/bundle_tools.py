...
from verumtrade.agents.utils.market_data.price_action_tools import get_price_action_summary
...
from verumtrade.agents.utils.market_data.technical_indicators_tools import get_indicators
from verumtrade.agents.utils.market_data.vwap_tools import (
    get_intraday_vwap_position,
    get_multi_day_vwap_context,
)
from verumtrade.agents.utils.market_data.fundamentals_parser import build_fundamentals_packet
from verumtrade.agents.utils.market_data.macro_regime import build_macro_regime_context
from verumtrade.agents.utils.market_data.peer_sets import resolve_peers
    ...
    *,
    ...
    for line in lines:
        ...
        if any(
            ...
            if (
                source in _TARGET_SCOPED_EVENT_SOURCES
                and relevance["decision"] == "drop"
                and not relevance.get("mentioned_tickers")
            ):
                relevance = {
                    **relevance,
                    "relevance_score": max(float(relevance.get("relevance_score") or 0.0), 0.70),
                    "decision": "accept",
                    "matched_aliases": relevance.get("matched_aliases") or [ticker],
                    "contamination_flags": [
                        flag
                        for flag in relevance.get("contamination_flags", [])
                        if flag != "unrelated_ticker_without_target"
                    ],
