...
``verumtrade.agents.discovery.intelligence.market_context_snapshot.PreStage0IntelligenceBuilder``
    ...
    *,
    ...
    try:
        ...
        if not bool(cfg.get("enable_macro_regime_context", True)):
            return {}
        if snapshot is None:
            from verumtrade.agents.discovery.intelligence.market_context_snapshot import (
...
def summarize_macro_regime(
    ...
    try:
        from verumtrade.agents.utils.market_data.narrative_catalyst import tag_narrative_events
...
def extract_macro_events(regime: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    ``verumtrade.schemas.catalyst_events.MacroEventRecord.from_dict``.
    """
