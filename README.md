...
> [!TIP]
> Run `tradingagents --help` (or `python -m cli.main --help`) to see every command and option.
...
- **Reasoning trace** — for each agent, *what it looked at and how it concluded*. Built by
  [`graph/reasoning_trace.py`](tradingagents/graph/reasoning_trace.py) and surfaced in the
...
- **Decision trace** — the chain from evidence → research debate → trader plan → risk review →
  ...
  a structured object validated against [`graph/decision_schema.py`](tradingagents/graph/decision_schema.py)
  and extracted by [`graph/signal_processing.py`](tradingagents/graph/signal_processing.py).
...
**structured evidence graph** ([`agents/utils/agent_runtime/evidence_graph.py`](tradingagents/agents/utils/agent_runtime/evidence_graph.py))
...
**Journal** subsystem ([`tradingagents/agents/journal/`](tradingagents/agents/journal/)) closes the
...
See [`tradingagents/agents/journal/USAGE.md`](tradingagents/agents/journal/USAGE.md) and
...
A complementary **Theme Engine** ([`agents/discovery/theme_engine/`](tradingagents/agents/discovery/theme_engine/))
...
| **HTTP 400 — context length exceeded** | The model's context window is too small for the prompt. Keep `TRADINGAGENTS_CONTEXT_BUDGET_MODE=adaptive` (default) or set it to `compact`; avoid `off` on strict models. |
| **HTTP 429 — rate limited** | Your provider is throttling. Use a smaller/faster model, lower research depth, or raise the manager delay knobs (`TRADINGAGENTS_RESEARCH_MANAGER_MIN_DELAY_S`, `TRADINGAGENTS_RISK_MANAGER_MIN_DELAY_S`). |
...
Run `tradingagents --help` to discover every command and flag.
